	...
	#	res.append(i.get('imageUrl'))
		for r in res:
			...
			if d.status_code == 200:
				obj = Logo.objects.create(name=serializer.data["name"],desc=serializer.data["desc"],color=c,img=base64.b64encode(d.content))
				obj.save()
...
def exist(serializer):
			...
			values = Logo.objects.filter(desc=serializer.validated_data['desc'],color=serializer.validated_data['color']).values('img')
			lis=[]
